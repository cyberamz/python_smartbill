...
class SmartBillTest(TestCase):
    ...
    def setUp(self):
        self.smartbill = SmartBill(SMARTBILL_USER, SMARTBILL_TOKEN, SMARTBILL_CIF, "USD",
                                   "RO", True, True)
    ...
    def test_create_invoice_payment_no_use_stock(self):
        product_list = []
        product_list.append(self.smartbill.create_product(name_product='Cyber Product 3', code='7X-VFIS-GSXB',
                                           measuring_unit_name='buc', quantify=10, price=Decimal("10.02"),
                                           is_tax_included=True,
                                           tax_name='TVA Inclus',
                                           tax_percentage=Decimal('0.00'),
                                           is_service=False, use_stock=True, ware_house_name='CyberAmz'))
        stock = self.smartbill.get_stock(date='2018-05-29', warehouseName='CyberAmz')
        print(stock)
        ...
        client = self.smartbill.create_client(name_client='test', country_client="Romania")
        payment = self.smartbill.create_all_payment(product_list, is_cash=False)
        invoice = self.smartbill.create_invoice(client=client,
